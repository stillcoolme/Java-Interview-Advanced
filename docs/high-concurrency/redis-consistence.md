...
更新数据的时候，根据**数据的唯一标识**，将操作发送到一个 jvm 内部队列中。

读取数据的时候，如果发现数据不在缓存中，也将**重新读取数据+更新缓存**的操作，根据唯一标识路由之后，也发送同一个 jvm 内部队列中。
...
如果**读取请求**还在等待时间范围内，不断轮询发现可以取到值了，那么就直接返回；如果请求等待的时间超过一定时长，那么这一次直接从数据库中读取当前的旧值。
